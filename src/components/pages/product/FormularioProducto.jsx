...
const FormularioProducto = ({ titulo, estoyCreando }) => {
  ...
  useEffect(() => {
    //si estoy no creando, es decir editando
    if (!estoyCreando) {
  ...
  const productoValidado = async (producto) => {
    if (estoyCreando === true) {
      //pedir a la api crear mi producto
      const respuesta = await crearProductoApi(producto);
      console.log(respuesta);
      if (respuesta.status === 201) {
        Swal.fire({
          title: "Producto Creado!",
          text: `El producto ${producto.nombreProducto}, fue creado con éxito!`,
          icon: "success",
        });
        reset();
      } else {
        Swal.fire({
          ...
          text: `El producto ${producto.nombreProducto}, no pudo ser creado, reintanta en unos minutos.`,
          icon: "error",
        });
      }
    } else {
      ...
      if (respuesta === 200) {
        ...
        Swal.fire({
          title: "Ocurrio un error!",
          text: `El producto ${producto.nombreProducto}, no pudo ser modificado, reintanta en unos minutos.`,
          icon: "error",
        });
      }
    }
